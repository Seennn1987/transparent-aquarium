...
async function start() {
  ...
  // 透明標本アクアリウム: 元の魚群は既定で非表示。?fish=1 を付けると元の魚を表示。
  // 透明標本を入れるまでは、水槽環境（水草・流木・岩・水・光）だけを動かす。
  const SHOW_ORIGINAL_FISH = new URLSearchParams(location.search).get("fish") === "1";
  const fish = SHOW_ORIGINAL_FISH
